...
public class DeviceClient
{
    ...
    {
        var task = Task.Run(() => GetDeviceInfoAsync(registrationId));
        task.Wait();
        return task.Result;
    }
    ...
    public async Task<HttpResponse> UpdateDeviceInfoAsync(String registrationId, String json)
    {
        ...
        var msg = await JPushClient.HttpClient.PostAsync(url, requestContent).ConfigureAwait(false);
    ...
    public async Task<HttpResponse> GetDevicesByAliasAsync(String alias, String platform)
    {
        ...
        var msg = await JPushClient.HttpClient.GetAsync(url).ConfigureAwait(false);
        var responseConetent = await msg.Content.ReadAsStringAsync().ConfigureAwait(false);
    ...
    {
        var task = Task.Run(() => GetDevicesByAliasAsync(alias, platform));
        task.Wait();
        return task.Result;
    }
    ...
    {
        var task = Task.Run(() => DeleteAliasAsync(alias, platform));
        task.Wait();
        return task.Result;
    }
    ...
    {
        var task = Task.Run(() => GetTagsAsync());
        task.Wait();
        return task.Result;
    }
    ...
    {
        var task = Task.Run(() => IsDeviceInTagAsync(registrationId, tag));
        task.Wait();
        return task.Result;
    }
    ...
    public async Task<HttpResponse> AddDevicesToTagAsync(String tag, List<String> registrationIdList)
    {
        ...
        if (registrationIdList == null || registrationIdList.Count == 0)
            throw new ArgumentException(null, nameof(registrationIdList));

        var url = BASE_URL + "/v3/tags/" + tag;

        var jObj = new JsonObject
        {
            ["registration_ids"] = new JsonObject
            {
                ["add"] = new JsonArray([.. registrationIdList.Select(id => JsonValue.Create(id))])
            }
        };
    ...
    {
        var task = Task.Run(() => AddDevicesToTagAsync(tag, registrationIdList));
        task.Wait();
        return task.Result;
    }
    ...
    public async Task<HttpResponse> RemoveDevicesFromTagAsync(String tag, List<String> registrationIdList)
    {
        ...
        var jObj = new JsonObject
        {
            ["registration_ids"] = new JsonObject
            {
                ["remove"] = new JsonArray([.. registrationIdList.Select(id => JsonValue.Create(id))])
            }
        };
    ...
    {
        var task = Task.Run(() => RemoveDevicesFromTagAsync(tag, registrationIdList));
        task.Wait();
        return task.Result;
    }
    ...
    {
        var task = Task.Run(() => DeleteTagAsync(tag, platform));
        task.Wait();
        return task.Result;
    }
    ...
    public async Task<HttpResponse> GetUserOnlineStatusAsync(List<String> registrationIdList)
    {
        ...
        var jObj = new JsonObject
        {
            ["registration_ids"] = new JsonArray([.. registrationIdList.Select(id => JsonValue.Create(id))])
        };
    ...
    {
        var task = Task.Run(() => GetUserOnlineStatusAsync(registrationIdList));
        task.Wait();
        return task.Result;
    }
}