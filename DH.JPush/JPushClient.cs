...
public class JPushClient
{
    ...
    public async Task<HttpResponse> SendPushAsync(string jsonBody)
    {
        ...
        var msg = await HttpClient.PostAsync(BASE_URL, httpContent).ConfigureAwait(false);
    ...
    {
        var task = Task.Run(() => SendPushAsync(pushPayload));
        task.Wait();
        return task.Result;
    }
    ...
    public async Task<HttpResponse> IsPushValidAsync(String jsonBody)
    {
        ...
        var msg = await HttpClient.PostAsync(url, httpContent).ConfigureAwait(false);
        var content = await msg.Content.ReadAsStringAsync().ConfigureAwait(false);
        return new HttpResponse(msg.StatusCode, msg.Headers, content);
    }
    ...
    {
        var task = Task.Run(() => IsPushValidAsync(pushPayload));
        task.Wait();
        return task.Result;
    }

    /// <summary>
    /// <see cref="GetCIdList(Int32?, String)"/>
    /// </summary>
    public async Task<HttpResponse> GetCIdListAsync(Int32? count, String type)
    {
        ...
        var msg = await HttpClient.GetAsync(url).ConfigureAwait(false);
        var content = await msg.Content.ReadAsStringAsync().ConfigureAwait(false);
        return new HttpResponse(msg.StatusCode, msg.Headers, content);
    }
    ...
    public HttpResponse GetCIdList(Int32? count, String type)
    {
        var task = Task.Run(() => GetCIdListAsync(count, type));
        task.Wait();
        return task.Result;
    }
    ...
    {
        var task = Task.Run(() => BatchPushByRegidAsync(singlePayLoadList));
        task.Wait();
        return task.Result;
    }
    ...
    {
        var task = Task.Run(() => BatchPushByAliasAsync(singlePayLoadList));
        task.Wait();
        return task.Result;
    }
    ...
    {
        var cidResponse = await GetCIdListAsync(singlePayLoadList.Count, "push").ConfigureAwait(false);
        var jsonNode = JsonNode.Parse(cidResponse.Content);
        var cidArray = jsonNode?["cidlist"]?.AsArray();

        var batchPushPayload = new BatchPushPayload();
        batchPushPayload.Pushlist = new Dictionary<String, SinglePayload>();
        for (var i = 0; i < singlePayLoadList.Count; i++)
        {
            var cid = cidArray?[i]?.ToString();
            batchPushPayload.Pushlist.Add(cid, singlePayLoadList[i]);
        }
